...
class CollectApp:
    ...
    def run(self) -> int:
        ...
        try:
            while True:
                ...
                if self.capturing and self._last_hand is not None and self._frame_counter % 2 == 0:
                    feat = landmarks_to_features(self._last_hand)
                    append_sample(DATA_PATH, self.label, feat)
    ...
    def _draw(self, frame: np.ndarray, hands, w: int, h: int) -> np.ndarray:
        ...
        hand_state = "hand visible" if self._last_hand is not None else "show your hand"
